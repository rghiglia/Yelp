...
#len(y_trn)
#nTrn
## You lost one on the way!
# Now ok, use test_size = nTst-1
...
sss = StratifiedShuffleSplit(y_trn, nCV, test_size=nCV_tst-1, random_state=1)
for i, (train_index, test_index) in enumerate(sss):
    if i==0:
        nCV_trn = len(train_index)
        nCV_tst_sss = len(test_index)
# The method does NOT!!! pick exactly nCV_tst!!!
nCV_tst = nCV_tst_sss
...
#    if i==0:
#        tmp.append(train_index)
#        print len(train_index)
#        print max(train_index)
#        print y_trn.index[max(train_index)]
#        for j in train_index:
#            print 'j={}, ix={}'.format(j, y_trn.index[j])
...
# DAMN! I think the problem is:
nTrn-nCV_tst = 18115
len(train_index) = 18113
len(y_trn)-nCV_tst = 18114

tmp[0].sort()
tmp[0][:10]
tmp[0].max()

tmp_trn = y_trn.index.copy()
tmp_trn = tmp_trn.sort_values()
tmp_trn[-10:]
# Ok, so these are in the orginal index
