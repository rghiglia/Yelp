...
rev_CV_cln_trn = []
for i in range(nCV_trn):
    print 'Review {}/{}'.format(i+1, nCV_trn)
    rev_CV_cln_trn.append(clean_review(df_rev_CV_trn['text'].iloc[i]))

# Test
#nR = len(df_rev_CV_tst)
rev_CV_cln_tst = []
for i in range(nCV_tst):
    print 'Review {}/{}'.format(i+1, nCV_tst)
    rev_CV_cln_tst.append(clean_review(df_rev_CV_tst['text'].iloc[i]))
...
#                             max_features = None)
vectorizer = CountVectorizer(analyzer='word')
print 'Bag of words: generation and transformation of training set ...'
X_CV_trn = vectorizer.fit_transform(rev_CV_cln_trn)
print 'Ok'
X_CV_trn = X_CV_trn.toarray()
vocab = vectorizer.get_feature_names()
print vocab

# VERY CAREFUL HERE: only transform!
print 'Bag of words: transformation of testing set ...'
X_CV_tst = vectorizer.transform(rev_CV_cln_tst)
print 'Ok'
X_CV_tst = X_CV_tst.toarray()
#vocab = vectorizer.get_feature_names()
#print vocab
...
X_CV_trn.shape
X_CV_tst.shape
# They should have the same columns (ergo also same number)
...
score = f1_score(y_CV_tst, y_CV_pred)
print 'Score = {}'.format(score)

# Benchmark
float(len(y_CV_tst[y_CV_tst==1])) / len(y_CV_tst)

# Slightly better than BM

# Ok, with max_depth=6 I pushed it a bit higher




# Now use CV to optimize

# Select CV train and CV test sets
mdv = range(3,9)
nPrm = len(mdv)
S = pd.DataFrame(np.zeros((nCV, nPrm)), columns=mdv)    # score
B = pd.DataFrame(np.zeros((nCV, nPrm)), columns=mdv)    # benchmark
T = pd.DataFrame(np.zeros((nCV, nPrm)), columns=mdv)    # timing
M = {m: np.zeros((2,2)) for m in mdv}

for iCV in range(nCV):

    print '------------- CV {}/{}'.format(iCV+1, nCV)
    y_CV_trn = y_trn.ix[Ix_trn[:,iCV]]
    y_CV_tst = y_trn.ix[Ix_tst[:,iCV]]
    
    df_rev_CV_trn = df_rev.ix[Ix_trn[:,iCV]]
    df_rev_CV_tst = df_rev.ix[Ix_tst[:,iCV]]
    
    # Train
    rev_CV_cln_trn = []
    for i in range(nCV_trn):
        print 'Cleaning review {}/{}'.format(i+1, nCV_trn)
        rev_CV_cln_trn.append(clean_review(df_rev_CV_trn['text'].iloc[i]))
    
    # Test
    rev_CV_cln_tst = []
    for i in range(nCV_tst):
        print 'Cleaning review {}/{}'.format(i+1, nCV_tst)
        rev_CV_cln_tst.append(clean_review(df_rev_CV_tst['text'].iloc[i]))
    ...
    for md in mdv:
        ...
        y_CV_pred = clf.predict(X_CV_tst)
        t_en = time.time()
        print '\t\t Time = {}'.format(t_en-t_in)
        
        S.ix[iCV, md] = f1_score(y_CV_tst, y_CV_pred)
        B.ix[iCV, md] = float(len(y_CV_tst[y_CV_tst==1])) / len(y_CV_tst)
        T.ix[iCV, md] = t_in - t_en
        M[md] = confusion_matrix(y_CV_tst, y_CV_pred)
        print '\t\t Score = {}'.format(S.ix[iCV, md])
